...
parser.add_argument('--api_key', help='the key for accessing APIs', default="3Opabtz5OBXZzZk9aGWe")
...
###
#test /v0.2/Cache/Detect
###
print('** testing /v0.2/Cache/Detect **')
url = "{}{}".format(server, '/v0.2/Cache/Detect')
data = {        
    "imgData":base64_data['friends'],
}
response = requests.post(url.format(server), headers = headers, data=json.dumps(data), proxies=proxies)
print('response:')
print(response.text)
...
cacheFaceIds = [face['cacheFaceId'] for face in rdata['faces']] # get cacheFaceIds for further testing



###
#test /v0.2/Cache/Match
###
print('** testing /v0.2/Cache/Match **')
url = "{}{}".format(server, '/v0.2/Cache/Match')
data = {                
            "queryCacheFaceIds":cacheFaceIds[2:4],    #take the second largest face as the query face
            "candidateCacheFaceIds":cacheFaceIds[:]    #take all faces as the candidate faces
        }
response = requests.post(url.format(server), headers = headers, data=json.dumps(data), proxies=proxies)
print('response:')
print(response.text)
